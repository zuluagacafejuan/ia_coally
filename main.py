...
#################### FUNCIONES ÚTILES ###################
#########################################################
print(os.environ['MONGO_COALLY'])
client = MongoClient(os.environ['MONGO_COALLY'])
...
def obtener_vectores_oportunidades_id(id, uniandes=False):
  ...
  columnas = [desc[0] for desc in cursor.description]
  df_resultados = pd.DataFrame(resultados, columns=columnas)
  return df_resultados.drop(['cluster', 'approved_by', 'id', 'experiencia', 'softskills', 'hardskills', 'carrera', 'tipo_oferta'], axis = 1)
...
def descargar_data_cv(id_cv, uniandes =False):
  if uniandes:
    client = MongoClient(os.environ['MONGO_UNIANDES']) 
...
def obtener_mejores_oportunidades_similitud(cluster, vector_cv, uniandes=False):
  ...
  if len(vectores_oportunidades_cluster) == 0:
    return {}
  similitud_cos = cosine_similarity(vector_cv, vectores_oportunidades_cluster.drop(['cluster', 'id', 'experiencia', 'softskills', 'hardskills', 'carrera', 'approved_by', 'tipo_oferta'], axis = 1))
...
def agregar_proyecto(id_proyecto, uniandes=False):
  ...
  features_proyecto['tipo_oferta'] = tipo_oferta
  for index, item in enumerate(vector):
...
def agregar_aplicante(id_job, id_cv, uniandes = False):
  ...
  similitud = cosine_similarity(cv_df.drop(['cluster', 'id', 'experiencia', 'softskills', 'hardskills', 'carrera'], axis = 1), oportunidad_df.drop(['cluster', 'tipo_oferta', 'approved_by', 'id', 'experiencia', 'softskills', 'hardskills', 'carrera'], axis = 1))[0][0]
